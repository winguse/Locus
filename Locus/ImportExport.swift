...
class ImportExport {
  ...
  static func exportData(override: Bool = false) {
    PersistenceController.shared.container.performBackgroundTask { _ in

      let pc = PersistenceController.shared
      let calendar = Calendar.current
      let oneDay = 3600 * 24.0
      let endTime = Date(timeIntervalSinceNow: 0)
    ...
    PersistenceController.shared.container.performBackgroundTask { ctx in
      ...
      do {
        ...
        for file in files {
          ...
          var matrix = Array(repeating: Array<Double?>.init(repeating: nil, count: headers.count), count: lines.count - 1)
          ...
          if override {
            ...
            let start = Date(timeIntervalSince1970: allTs.min()!)
            let end = Date(timeIntervalSince1970: allTs.max()!)
          ...
          for row in matrix {
            ...
            newLocation.timestamp = Date(timeIntervalSince1970: row[0]!)
